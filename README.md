...
- **Clusters** (unlisted for now, at [/cluster/](https://p.migdal.pl/genetic-distance-map/cluster/)): Ward hierarchical clustering of the present-day populations on the same coordinates. Every split in the panel is a real branch of that tree; the map is coloured by the cluster of the nearest sampled location. Admixed groups sit on long branches between clades, which is a limit of trees, not a fact about history.
