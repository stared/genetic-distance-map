...
import { createMap, loadWorld, setPoints, setSelection, fitTo, repaintHeat, setResampling } from './mapview'
...
async function main() {
  ...
  function renderSim() {
    refreshVisible(); setResampling(map, 'linear')
    if (!query) { heat.clear(); setPoints(map, pops, p => visible[p.id] ? '#b4b4bb' : null, () => 0.9); repaintHeat(map); $('nearest').innerHTML = ''; setSelection(map, []); return }
    const dmax = +dmaxEl.value
    for (const p of pops) d[p.id] = 100 * dist(query.c, p.c)
    heat.renderHeat(d, visible, dmax); repaintHeat(map)
  ...
  function paintCategories(assign: Map<number, number>, colors: string[], inScope: (p: Pop) => boolean) {
    ...
    heat.renderCategories(locCat, pal); setResampling(map, 'linear'); repaintHeat(map); setSelection(map, [])
    setPoints(map, pops, p => { if (!visible[p.id]) return null; const c = assign.get(p.id); return c !== undefined ? colors[c] : inScope(p) ? '#b4b4bb' : null }, p => assign.has(p.id) ? 1 : 0.5)
  }
  ...
  function renderClusters() {
    ...
    const opts: Parameters<typeof renderTree>[1] = { tree, root: tree.root, cutRoots: new Set(roots), colors, byId, expanded, onSelect: n => fitTo(map, tree.members(n).map(id => byId.get(id)!)), onToggle: () => renderTree($('clutree'), opts) }
  ...
  map.on('click', e => {
    ...
    if (mode === 'drill') {
      if (!drill) return
      let c = f ? drill.assign.get(f.properties.id as number) : undefined
      if (c === undefined) { // click on a coloured region: nearest visible assigned pop
        const near = pops.filter(p => p.lat != null && visible[p.id] && drill!.assign.has(p.id)).map(p => ({ p, km: haversine(e.lngLat.lat, e.lngLat.lng, p.lat!, p.lon!) })).sort((a, b) => a.km - b.km)[0]
        if (near && near.km < 800) c = drill.assign.get(near.p.id)
      }
